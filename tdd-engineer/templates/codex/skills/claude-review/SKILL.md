...
Run Claude Code locally to review changes related to a GitHub issue. Claude will inspect the issue, run git status / git diff as needed, and return a structured review.
...
- Runs Claude Code in non-interactive mode to analyze changes
- Prints the structured review output
...
```bash
bash .claude/skills/claude-review/run.sh <issue-number>
```
